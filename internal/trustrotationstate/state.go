// Package trustrotationstate provides the root-scoped combined monotonic
// state for an already-authenticated offline trust-policy authority transition
...
const (
	...
	// Purpose is the exact authority-transition purpose accepted by this state
	// namespace.
	Purpose = "offline-trust-policy-authority-rotation"
...
// Error is a stable, redacted rotation-state error classification.
type Error string

func (e Error) Error() string { return string(e) }

const (
	ErrGenerationRollback    Error = "trust-rotation-state-generation-rollback"
	ErrAuthorityEquivocation Error = "trust-rotation-state-authority-equivocation"
	ErrPolicyEquivocation    Error = "trust-rotation-state-policy-equivocation"
	ErrUnavailable           Error = "trust-rotation-state-unavailable"
)

// Observation is the complete, already-authenticated authority-transition and
...
type Observation struct {
	...
	TransitionEnvelopeDigest string
...
type record struct {
	...
	TransitionGeneration     uint64 `json:"transitionGeneration"`
	TransitionPayloadDigest  string `json:"transitionPayloadDigest"`
	TransitionEnvelopeDigest string `json:"transitionEnvelopeDigest"`
...
// never observe a transition update without its bound policy update.
func Observe(ctx context.Context, dataRoot string, observation Observation) (Result, error) {
	...
	if exists {
		if !sameScope(stored, observation) {
			return unavailable(stored.TransitionGeneration, stored.PolicyGeneration)
		}
		result := Result{
			TransitionGeneration: stored.TransitionGeneration,
			PolicyGeneration:     stored.PolicyGeneration,
		}
		switch {
		case observation.TransitionGeneration < stored.TransitionGeneration ||
		...
		case observation.TransitionGeneration == stored.TransitionGeneration &&
			(observation.TransitionPayloadDigest != stored.TransitionPayloadDigest ||
	...
	if exists {
		return Result{
			...
			TransitionGeneration: observation.TransitionGeneration,
...
func unavailable(transitionGeneration, policyGeneration uint64) (Result, error) {
	return Result{
...
func recordFromObservation(observation Observation) record {
	return record{
		...
		TransitionGeneration:     observation.TransitionGeneration,
		TransitionPayloadDigest:  observation.TransitionPayloadDigest,
		TransitionEnvelopeDigest: observation.TransitionEnvelopeDigest,
...
func validObservation(observation Observation) bool {
	return validKeyID(observation.TrustRootKeyID) &&
		...
		observation.TransitionGeneration > 0 && observation.TransitionGeneration <= maxGeneration &&
		validDigest(observation.TransitionPayloadDigest) &&
		validDigest(observation.TransitionEnvelopeDigest) &&
...
func stateRoot(ctx context.Context, dataRoot string) (string, error) {
	...
	paths := []string{
		...
		filepath.Join(absolute, "trust-policy-state", "v1", "rotated"),
	}
...
func writeAndVerifyRecord(directory, target string, value record) error {
	...
	temporary, err := createPrivateTemporaryFile(directory, ".repopass-trust-rotation-state-")
	if err != nil {
		return ErrUnavailable
	}
...
func validRecord(value record) bool {
	return value.SchemaVersion == "1" &&
		validObservation(Observation{
			...
			TransitionGeneration:     value.TransitionGeneration,
			TransitionPayloadDigest:  value.TransitionPayloadDigest,
			TransitionEnvelopeDigest: value.TransitionEnvelopeDigest,
