...
func testPlatformSecurityAndAtomicity(t *testing.T) {
	t.Run("reparse data root", func(t *testing.T) {
		base := t.TempDir()
	...
	t.Run("created objects are private", func(t *testing.T) {
		...
		for _, path := range []string{filepath.Join(stateDirectory, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.lock"), stateFileForTest(t, root)} {
