...
func requireGateEnvironment(t *testing.T, environment []string, configuration gateRunEnvironment, specification GateSpec) {
	...
	want := map[string]string{
		...
		"CGO_ENABLED": "0",
		"GOCACHE":     configuration.GoCacheDir, "GOMODCACHE": configuration.GoModCacheDir,
		"GOPATH": filepath.Join(configuration.HomeDir, "go"), "GOBIN": filepath.Join(configuration.HomeDir, "bin"),
